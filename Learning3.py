'''
Learning3는 main.py와 다른 방식으로 encoding을 진행하려고 함.
input이 spike 에서 spike train으로 방법을 변경함.
우선 Linear한 데이터로 진행해 보았음. 2024.01.02. KDH
'''
import os
import numpy as np
...
def run_snn2(inputs):
    ...
    # Readout layer
    h2 = torch.einsum("abc,cd->abd", (remake_spk, w2))
...
def save_to_csv(w1, w2, w3, w4, loss_hist):
    ...
    file_name = f"weights_{current_time}_l3.csv"
